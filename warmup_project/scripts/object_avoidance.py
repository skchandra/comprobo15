...
class ObjectAvoid(object):
	...
	def processScan(self, msg):
		...
		ang = len(scanRightFront) - len(scanLeftFront) * .01 #proportional control
		#dist = .05
		
		if (len(scanRightBack)+len(scanLeftBack)) > 0: #if something behind robot
			dist = (len(scanRightBack)+len(scanLeftBack) - 1) * .05 #proportional control
		else: 
			dist = .1
